...
function updateTimeSlots() {
  ...
  // Функція для нормалізації часу – повертає рядок "HH:mm"
  ...
  // Додаємо часову мітку, щоб не кешувалось
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      // Фільтруємо записи для вибраної дати (Google Apps Script повертає масив рядків)
      ...
      // Ми відобразимо усі слоти, але заброньовані та вже минулі (якщо вибрана сьогоднішня дата) зробимо неактивними
      ...
      allSlots.forEach(slot => {
        ...
        // Якщо слот вже пройшов або заброньований, робимо його неактивним
        if (isPast || isBooked) {
          option.disabled = true;
        }
        timeSelectElem.appendChild(option);
...
function bookAppointment() {
  ...
  fetch('https://script.google.com/macros/s/AKfycbx_Sjqds2oIId57hsSTh2tgDTY8NuW6MxoBEYc5g3VhRC9dlumHhch0q1INORNVcoy3/exec', {
    ...
    .then(result => {
      ...
      // Можливо, варто зробити незначну затримку перед оновленням слотів, щоб дані в Google Sheet встигли оновитись
